...
# 3. Status enum identical across SKILL.md, workflow.md, INDEX.template.md.
enum_values="planning in-progress review blocked done abandoned"
enum_mismatch=""
for f in "$SKILL_MD" "$REFS/workflow.md" "$REFS/INDEX.template.md"; do
  for v in $enum_values; do
    grep -q "\`$v\`" "$f" || enum_mismatch="$enum_mismatch $f:missing[$v]"
  done
done
report "$([ -z "$enum_mismatch" ]; echo $?)" "status enum values present in all three files" "$enum_mismatch"
...
# 5. Plugin manifests: parseable, names consistent with the skill layout.
plugin_ok=1
...
# 6. Codex agents/openai.yaml metadata is complete.
openai_yaml_ok=1
...
# 8. Dedicated agent definition preloads the skill.
...
# 9. SKILL.md frontmatter sanity: description word count in a reasonable range.
...
# 10. No leftover Kdan-specific coupling residue in the parts of the skill
...
# 11. Trigger matrix shape (if present).
...
# 12. Unit + integration test suite passes.
pytest_ok=1
...
# 13. Eval scenario grader regression tests pass (if present).
